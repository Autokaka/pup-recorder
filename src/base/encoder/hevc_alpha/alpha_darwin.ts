...
export function patchHevcAlphaPtl(bitstream: Buffer): Buffer {
  ...
  for (const nal of nals) {
    let data = nal.data;
    if (nal.type === NAL_VPS) {
      data = patchPtl(nal.data, VPS_PTL_BIT);
    } else if (nal.type === NAL_SPS) {
      data = patchPtl(nal.data, SPS_PTL_BIT);
    }
...
function rewriteOneUe(nalIn: Buffer, bitOffset: number, newVal: number): Buffer {
  ...
  for (let i = 0; i < bitOffset; i++) {
    bw.bits.push(br.read(1));
  }
  br.readUe();
  bw.ue(newVal);
  while (br.pos < br.bits.length) {
    bw.bits.push(br.read(1));
  }
...
function parseInlineStRefPicSet(br: BitReader, numDeltaPocsSet0: number): void {
  ...
  if (interFlag) {
    ...
    if (deltaIdxMinus1 !== 0) {
      throw new Error(`apple_alpha: unexpected delta_idx_minus1=${deltaIdxMinus1}`);
    }
...
function parseHeaderToAlignment(br: BitReader, nalType: number, cfg: NvencHevcConfig): number {
  ...
  if (br.read(1) !== 1) {
    throw new Error("apple_alpha: expected first_slice_segment_in_pic_flag=1");
  }
  const isIrap = nalType >= NAL_BLA_W_LP && nalType <= NAL_RSV_IRAP_VCL23;
  if (isIrap) {
    br.read(1); // no_output_of_prior_pics_flag
  }
  ...
  if (!isIrap) {
    ...
    if (!stRefPicSetSpsFlag) {
      parseInlineStRefPicSet(br, cfg.numDeltaPocsSet0);
    } else if (cfg.numShortTermRefPicSets > 1) {
      br.read(Math.ceil(Math.log2(cfg.numShortTermRefPicSets)));
    }
    if (cfg.longTermRefPicsPresent) {
      throw new Error("apple_alpha: long_term_ref_pics path unimplemented");
    }
    if (cfg.spsTemporalMvpEnabled) {
      br.read(1); // slice_temporal_mvp_enabled_flag
    }
  }
  if (cfg.saoEnabled) {
  ...
  if (sliceType === SLICE_TYPE_P || sliceType === SLICE_TYPE_B) {
    ...
    if (sliceType === SLICE_TYPE_B) {
      br.read(1); // mvd_l1_zero_flag
    }
    if (cfg.cabacInitPresent) {
      br.read(1); // cabac_init_flag
    }
    br.readUe(); // five_minus_max_num_merge_cand
  }
  readSe(br); // slice_qp_delta
  if (cfg.ppsHasLoopFilterAcrossSlicesFlag) {
    br.read(1); // slice_loop_filter_across_slices_enabled_flag
  }
  return br.pos;
}
...
export function rewriteAlphaSliceHeader(slice: Buffer, nalType: number, cfg: NvencHevcConfig): Buffer {
  ...
  const bw = new BitWriter();
  for (let i = 0; i < 16; i++) {
    bw.bits.push(br.read(1)); // NAL header
  }
  bw.bits.push(br.read(1)); // first_slice_segment_in_pic_flag
  const isIrap = nalType >= NAL_BLA_W_LP && nalType <= NAL_RSV_IRAP_VCL23;
  if (isIrap) {
    bw.bits.push(br.read(1)); // no_output_of_prior_pics_flag
  }
  ...
  while (br.pos < B) {
    bw.bits.push(br.read(1)); // copy rest of header bits
  }

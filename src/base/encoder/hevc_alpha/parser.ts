...
function parseSps(spsRbsp: Buffer): {
  ...
  if (maxSubLayers !== 0) {
    fail("sps_max_sub_layers_minus1", maxSubLayers, "0");
  }
  ...
  if (chromaFormat === 3) {
    fail("chroma_format_idc", chromaFormat, "≠3");
  }
  ...
  br.readUe();
  if (br.read(1)) {
    fail("scaling_list_enabled_flag", 1, "0");
  }
  br.read(1); // amp_enabled_flag
  const saoEnabled = br.read(1) === 1;
  if (br.read(1)) {
    fail("pcm_enabled_flag", 1, "0");
  }
  const numStRps = br.readUe();
  if (numStRps > 1) {
    fail("num_short_term_ref_pic_sets", numStRps, "≤1");
  }
  ...
  if (longTermRefPicsPresent) {
    fail("long_term_ref_pics_present_flag", 1, "0");
  }
  const spsTemporalMvpEnabled = br.read(1) === 1;
  return {
    log2MaxPocLsb,
    saoEnabled,
    numShortTermRefPicSets: numStRps,
    numDeltaPocsSet0,
...
function parsePps(ppsRbsp: Buffer): { cabacInitPresent: boolean; ppsHasLoopFilterAcrossSlicesFlag: boolean } {
  ...
  if (numExtraSliceHeaderBits !== 0) {
    fail("num_extra_slice_header_bits", numExtraSliceHeaderBits, "0");
  }
  ...
  if (br.read(1)) {
    br.readUe(); // cu_qp_delta_enabled → diff_cu_qp_delta_depth
  }
...
export function parseNvencHevcConfig(extradata: Buffer): NvencHevcConfig {
  ...
  if (!sps) {
    throw new Error("hevc_alpha: SPS missing in NVENC extradata");
  }
  if (!pps) {
    throw new Error("hevc_alpha: PPS missing in NVENC extradata");
  }
